...
function randStr($num=6){
    $str = str_shuffle('abcedfghjkmnpqrstuvwxyzABCEDFGHJKMNPQRSTRUVWXYZ234567789');
...
function makeThumb($oimg, $sw=200, $sh=200){
    ...
    //7 = TIFF(inter byte order), 8 = TIFF(motorola byte order), 9 = JPC, 10 = JP2,
...
* 加密用戶名
*
* @param str $name 用戶登陆时输入的用戶名
* @return str md5(用戶名+salt)=>md5码
*/
function cCode($name){
